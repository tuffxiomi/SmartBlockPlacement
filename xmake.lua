...
target("SmartBlockPlacement")
    ...
    if is_plat("android") then
        add_cxflags("-fPIC", "-Oz", "-ffunction-sections", "-fdata-sections", "-flto", "-fno-unwind-tables", "-fno-asynchronous-unwind-tables", "-fmerge-all-constants", "-fno-stack-protector", "-fexceptions", "-fvisibility=hidden")
    ...
    after_build(function (target)
        ...
        assert(python, "Python 3 is required to package SmartBlockPlacement.levipack")
        ...
        table.insert(args, path.join(os.projectdir(), "assets", "smartblockplacement.png"))
        table.insert(args, "--metadata")
        table.insert(args, path.join(os.projectdir(), "include", "mod_metadata.hpp"))
        table.insert(args, "--output")
        table.insert(args, path.join(target:targetdir(), "SmartBlockPlacement.levipack"))
